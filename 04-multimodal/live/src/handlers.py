...
def parse_time_string(time_str: str | None) -> time | None:
    """Преобразует строку времени HH:MM:SS или HH:MM в объект datetime.time."""
    if not time_str:
        return None
    try:
...
def format_time_string(time_str: str | None) -> str:
    """Форматирует строку времени для отображения (HH:MM)."""
    if not time_str:
        return ""
...
async def cmd_transactions(message: Message):
    ...
    sorted_transactions = sorted(user_transactions, key=lambda t: (t.date, parse_time_string(t.time) or time(0, 0)), reverse=True)
    ...
    for i, t in enumerate(sorted_transactions, 1):
        ...
        time_str = f" {format_time_string(t.time)}" if t.time else ""
