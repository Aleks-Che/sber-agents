...
- `/help` — справка по доступным командам и примеры запросов.
- `/reset` — очистка истории диалога (бот «забудет» предыдущие сообщения).
- `/recipe <запрос>` — поиск рецепта по ключевым словам (будет реализовано в итерации 5).
