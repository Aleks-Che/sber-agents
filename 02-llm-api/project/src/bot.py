...
if sys.platform == 'win32':
    try:
        ...
        # Настройка кодировки ввода
        sys.stdin.reconfigure(encoding='utf-8')
    except Exception:
        pass

# Инициализация Rich консоли для красивого вывода с поддержкой UTF-8
...
SYSTEM_PROMPT = """Ты — креативный юморист."""
...
class ChatBot:
    ...
    def add_message(self, role: str, content: str):
        ...
        # Если история превышает лимит, используем суммаризацию
        if len(self.conversation_history) > MAX_MESSAGES:
            self.summarize_history()
    
    def summarize_history(self):
        """Суммаризовать длинную историю диалога."""
        # Определяем количество сообщений для суммаризации (оставляем 5 последних)
        messages_to_keep = 5
        ...
        if self.conversation_history and self.conversation_history[0]["role"] == "system":
            ...
            messages_to_summarize -= 1  # Вычитаем системное сообщение из подсчета
        ...
        try:
            ...
            # Формируем новую историю с резюме и последними сообщениями
            new_history = []
            
            # Добавляем системный промпт, если он был
            if system_message:
            ...
            new_history.append({
                "role": "system",
                "content": f"Предыдущая переписка: {summary}"
            })
            
            # Добавляем последние сообщения
            new_history.extend(self.conversation_history[-messages_to_keep:])
            
            # Обновляем историю
            self.conversation_history = new_history
            
        except Exception as e:
            # В случае ошибки просто обрезаем историю без суммаризации
            console.print(f"[yellow]⚠️  Не удалось создать резюме: {e}[/yellow]")
            console.print("[yellow]Обрезаю историю без суммаризации...[/yellow]")
            
            # Просто обрезаем историю, сохраняя системный промпт
            if system_message:
